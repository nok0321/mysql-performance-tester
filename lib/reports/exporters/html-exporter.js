...
<body>
    <div class="container">
        ...
        <div class="metadata">
            ...
            ${reportData.metadata.configuration ? `<p><strong>MySQL Host:</strong> ${reportData.metadata.configuration.database?.host || 'N/A'}</p>` : ''}
        </div>
     ...
     * 推奨事項のHTML生成
     */
    generateRecommendationsHTML(reportData) {
     ...
     * 並列テストメトリクスのHTML生成
     */
    generateParallelMetricsHTML(metrics) {
        if (!metrics) return '';
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align:right">${fs.completed}</td>
                        <td style="text-align:right">${fs.failed}</td>
                        <td style="text-align:right">${fs.successRate}</td>
                        <td style="text-align:right">${fs.latency?.mean ?? '-'}</td>
                        <td style="text-align:right">${fs.latency?.p50 ?? '-'}</td>
                        <td style="text-align:right;color:#e67e22"><strong>${fs.latency?.p95 ?? '-'}</strong></td>
                        <td style="text-align:right">${fs.latency?.p99 ?? '-'}</td>
                        <td style="text-align:right;color:#27ae60">${fs.latency?.min ?? '-'}</td>
                        <td style="text-align:right;color:#c0392b">${fs.latency?.max ?? '-'}</td>
                    </tr>`).join('')}
                </tbody>
            </table>
            `;
     ...
     * 統計のHTML生成
     */
    generateStatisticsHTML(statistics) {
     ...
     * 解釈のHTML生成
     */
    generateInterpretationHTML(interpretation) {
     ...
     * Buffer PoolのHTML生成
     */
    generateBufferPoolHTML(bufferPool) {
    ...
    /**
     * Performance SchemaのHTML生成
     */
    generatePerformanceSchemaHTML(ps) {
        if (!ps) return '';
